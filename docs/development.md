...
### 🤖 For AI-Assisted Development (GitHub Copilot Agents)

This project includes **6 specialized Copilot agents** in `.github/agents/` that demonstrate role-based AI assistance:

| Agent | What it does | Can modify |
|-------|-------------|------------|
| **Documentation Writer** | Updates docs and guides | `.md` files only |
| **Test Engineer** | Writes/fixes unit & E2E tests | Test files only |
| **Feature Analyst** | Analyzes requests, proposes improvements | Nothing (read-only) |
| **Security Reviewer** | OWASP audits, vulnerability scans | Nothing (read-only) |
| **DevOps Engineer** | CI/CD, Bicep, Docker, configs | Infra/workflow files |
| **Localization Expert** | Manages all 9 language translations | Translation files only |

Each agent inherits global project context from `.github/copilot-instructions.md` and has scoped file permissions to enforce separation of concerns.
...
az resource list --resource-group ZavaGiftExchange --output table
...
❓ **Questions?** → [Open an Issue](https://github.com/Azure-Samples/DevOpsEngineerPersona/issues)
