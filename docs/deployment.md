...
|-------------|----------------|-----------|---------|
| **PR** | `ZavaGiftExchange-pr-{number}` | Created on PR open, deleted on close | Isolated testing per PR |
| **QA** | `ZavaGiftExchange-qa` | Persistent | Pre-production validation (isolated) |
| **Production** | `ZavaGiftExchange` | Persistent | Live environment |
...
az group create --name ZavaGiftExchange-qa --location eastus2
```
...
az deployment group create \
  --resource-group ZavaGiftExchange-pr-123 \
...
|-----------|---------|-------------|
| `projectName` | ZavaGiftExchange | Base name for resources |
...
```bash
az group delete --name ZavaGiftExchange-pr-123 --yes --no-wait
```
...
```bash
# Delete production
az group delete --name ZavaGiftExchange --yes

# Delete QA
az group delete --name ZavaGiftExchange-qa --yes

# Delete any orphaned PR environments
az group list --query "[?starts_with(name, 'ZavaGiftExchange-pr-')].name" -o tsv | \
